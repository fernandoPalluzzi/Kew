...
```
kew.py collide -t1 <table_1> -t2 <table_2> [-a1] [-a2] [-a] [-s <attributes>] [options]

# ARGUMENTS
# -d     Index file generated by the "store" function.
# -t1    Input table 1.
# -t2    Input table 2.
# -a1    Join attribute from table 1. Not needed if -a is used or -j is set to "natural" (see below).
# -a2    Join attribute from table 2. Not needed if -a is used or -j is set to "natural" (see below).
